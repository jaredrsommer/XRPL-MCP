...
function initializeCustody() {
    try {
        ...
        if (wallets.length > 0) {
            console.error(
                `Custody initialized: ${wallets.length} wallet(s) registered`
            );
            for (const w of wallets) {
                console.error(
                    `  ${w.isDefault ? "*" : " "} ${w.name} (${w.address}) [${w.type}]`
                );
            }
        } else {
            console.error(
                "No wallets configured. Use connect-to-xrpl tool or set XRPL_SEED env var."
            );
        }
    } catch (error) {
        console.error(
            "Failed to initialize custody:",
